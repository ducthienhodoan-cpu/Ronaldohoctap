...
// Mo ta: Dong co 3D May Gap Thu tuong tac Joystick 4 huong phien ban moi hoan chinh voi Three.js cho Web App
...
let clawState = 'idle'; // 'idle', 'lowering', 'grabbing', 'lifting', 'moving_to_chute', 'dropping', 'returning'
...
const THEME_MAY_GAP = {
    'cute': { bg: 0x0f172a, clawColor: 0xec4899 },
    'dino': { bg: 0x064e3b, clawColor: 0x10b981 },
    'football': { bg: 0x1e3a8a, clawColor: 0x3b82f6 },
    'future': { bg: 0x311042, clawColor: 0x06b6d4 },
    'galaxy': { bg: 0x181825, clawColor: 0x8b5cf6 },
    'golden': { bg: 0x451a03, clawColor: 0xf59e0b }
};
...
function khoi_tao_gap_thu_3d_moi() {
    ...
    if (!clawScene3D) {
        ...
        // Dong thu 20 vat pham nam o day
        for (let i = 0; i < 15; i++) {
...
function moveJoystickClaw3D(dir) {
    if (clawIsOperating || !clawArm3D) return;
...
function bat_dau_gap_thu_web_moi() {
    ...
    const statusMsg = 'Tay gắp đang từ từ hạ xuống đống thú bông... 🦾';
...
function animateGapThu3DMoi() {
    ...
    if (clawState === 'lowering') {
        ...
        if (clawArm3D && clawArm3D.position.x < 0) {
            ...
            const msg = `CHÚC MỪNG! GẮP THÀNH CÔNG THÚ BÔNG VÀO KHAY THƯỞNG! Nhận +150 Roblox XP!`;
            const resDiv = document.getElementById('clawResult');
            const resModalDiv = document.getElementById('modalClawResult');
