        ...
        function switchSection(secId, btnEl) {
            try {
                ...
                if (secId === 'ielts') { try { if (typeof loadIeltsQuestions === 'function') loadIeltsQuestions(); } catch(e) {} }
                if (secId === 'luyen') { try { if (typeof loadPracticeQuiz === 'function') loadPracticeQuiz(); } catch(e) {} }
                if (secId === 'hoc') { try { if (typeof loadStudyContent === 'function') loadStudyContent(); } catch(e) {} }
                if (secId === 'formula') { try { if (typeof loadFormulas === 'function') loadFormulas(); } catch(e) {} }
                if (secId === 'minigames') { try { if (typeof drawWheelCanvas === 'function') drawWheelCanvas(); } catch(e) {} }
                if (secId === 'worldcup') { try { if (typeof renderWcTeams === 'function') renderWcTeams(); if (typeof khoi_tao_cup_vo_dich_3d === 'function') khoi_tao_cup_vo_dich_3d('canvasTrophyWc3D', 'world_cup'); } catch(e) {} }
                if (secId === 'c1') { try { if (typeof renderC1Clubs === 'function') renderC1Clubs(); if (typeof khoi_tao_cup_vo_dich_3d === 'function') khoi_tao_cup_vo_dich_3d('canvasTrophyC13D', 'champions_league'); } catch(e) {} }
                if (secId === 'plan') { try { if (typeof loadPlan === 'function') loadPlan(); } catch(e) {} }
                if (secId === 'stats') { try { if (typeof loadStats === 'function') loadStats(); } catch(e) {} }
                if (secId === 'notebook') { try { if (typeof loadNotebook === 'function') loadNotebook(); } catch(e) {} }
                if (secId === 'duaxe') { try { if (typeof initCanvasRaceGame === 'function') initCanvasRaceGame(); } catch(e) {} }
                if (secId === 'obby') {
                    try {
        ...
        function submitObbyLevelAnswer() {
            if (obby6Questions && obby6Questions[obbyStep]) {
                const fromStep = obbyStep;
                const toStep = obbyStep + 1;
                showObbySolutionWithJumpButton(fromStep, toStep);
            }
        }

        function bat_dau_gap_thu_web() {
            const resDiv = document.getElementById('modalClawResult');
            if (resDiv) {
                resDiv.innerText = "Đang hạ tay gắp thú... 🦾";
                resDiv.style.color = "#F59E0B";
                setTimeout(() => {
                    const rand = Math.random();
                    if (rand < 0.3) {
                        resDiv.innerText = "Rất tiếc! Tay gắp bị trượt rớt thú. Thử lại lượt sau!";
                        resDiv.style.color = "#EF4444";
                    } else if (rand < 0.7) {
                        resDiv.innerText = "GẮP THÀNH CÔNG GẤU BÔNG THƯỜNG! Thưởng +50 Roblox XP!";
                        resDiv.style.color = "#10B981";
                    } else if (rand < 0.95) {
                        resDiv.innerText = "GẮP THÀNH CÔNG THỎ BÔNG HIẾM! Thưởng +200 Roblox XP!";
                        resDiv.style.color = "#F59E0B";
                    } else {
                        resDiv.innerText = "GẮP THÀNH CÔNG RỒNG HUYỀN THOẠI! Thưởng +500 Roblox XP!";
