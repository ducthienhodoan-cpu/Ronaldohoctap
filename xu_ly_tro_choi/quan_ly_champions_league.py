...
def lay_vong_dau_champions_league(clb_user_ten="Real Madrid"):
    """Trả về 4 vòng đấu chính của Cúp C1 Champions League với CLB đối thủ ngẫu nhiên khác CLB của học sinh."""
    tat_ca_clb = [c["ten"] for c in lay_danh_sach_clb_champions_league() if c["ten"] != clb_user_ten]
    random.shuffle(tat_ca_clb)
    
    clb_v1 = tat_ca_clb[0] if len(tat_ca_clb) > 0 else "Bayern Munich"
    ...
    return [
        {"vong": 1, "ten": "Vòng Bảng Cúp C1", "doi_thu": clb_v1, "so_cau": 5},
...
def sinh_tran_dau_champions_league(vong_idx, ten_lop="Lớp 6", ten_mon="Toán", ten_chu_de="Chủ đề 1", clb_user_ten="Real Madrid"):
    """Sinh bộ câu hỏi sút phạt Penalty cho trận đấu Champions League chuyên biệt theo 1 Chủ đề duy nhất."""
    vong_list = lay_vong_dau_champions_league(clb_user_ten)
